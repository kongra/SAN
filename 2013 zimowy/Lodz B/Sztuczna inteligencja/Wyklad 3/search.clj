...
(defn beam-search
  ...
  collection."
  [start goal? adjacent cost-fn beam-width]
  (tree-search (list start) goal? adjacent
...
(defn iterative-widening-search
  ...
  ([start goal? adjacent cost-fn]
     (iterative-widening-search
      start goal? adjacent cost-fn 1 100))
  
  ([start goal? adjacent cost-fn beam-width max-beam-width]
     (when (<= beam-width max-beam-width)
       (let [result (beam-search start goal? adjacent
				 cost-fn beam-width)]
	 (if result
	   result
	   (recur start goal? adjacent cost-fn
		  (inc beam-width) max-beam-width))))))


(defn graph-search
  "Finds a state that satisfies goal? predicate. Starts with states
  and proceeds according to adjacent and combiner. It
  never considers the same state twice, keeping the set of already
  examined states."
  ([states goal? adjacent combiner]
     (graph-search states goal? adjacent combiner #{}))
  ([states goal? adjacent combiner examined-states]
     (cond (nil? states) nil
